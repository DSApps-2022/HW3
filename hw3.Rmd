...
The `spotify_songs` dataset was curated by [Kaylin Pavlik](https://www.kaylinpavlik.com/author/walkerkq/) from the [`spotifyr`](https://www.rcharlie.com/spotifyr/) package. It contains about 33K songs from 6 main genres (Electronic Dance Music, Latin, Pop, RnB, Rap, Rock), where each song has:
...
* Pay attention to labelling, legends, ticks - make your plots look good (but don't overdo it just because you can)
...
spotify_songs %>%
  ...
  labs(x = "", y = "", title="Modality by Genre", caption = "Data: ~33K Spotify songs") +
  guides(fill = FALSE) +
